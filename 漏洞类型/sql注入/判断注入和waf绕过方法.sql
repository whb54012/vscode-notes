...
`table_name`会被当成表单参数table_name而不是字符串,所以结果还是information_schema.table_name

--内联注释,mysql专属,会将/*!内容*/里的内容当成命令或者表单参数执行
select * from user /*!union*/ select password from user--内联注释绕过union关键字让mysql执行union语句
select * from user union select password from /*!user*/ 绕过正则匹配
